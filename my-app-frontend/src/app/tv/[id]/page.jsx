  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (liked) {
        ...
        const response = await fetch(`http://localhost:5000/users/me/favorites/${tvShow.id}`, {
          method: 'DELETE',
          headers: {
  ...
  const handleAddToWatchlist = async () => {
    ...
    try {
      ...
      if (inWatchlist) {
        ...
        const response = await fetch(`http://localhost:5000/users/me/watchlist/${tvShow.id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
