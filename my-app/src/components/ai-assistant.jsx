...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Hey there! 👋 I'm CineBot, your personal movie & TV guide. Ask me anything about movies, shows, actors, or what's trending! 🎬",
    },
  ])
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      try {
        ...
        const cached = localStorage.getItem("cinebot_suggestions")
        if (cached) {
        ...
        if (response.ok) {
          ...
          if (data.suggestions?.length === 4) {
            ...
            localStorage.setItem("cinebot_suggestions", JSON.stringify({
  ...
  return (
    ...
    <>
      ...
      <button
        ...
        className={cn(
          "fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg hover:bg-primary/90 transition-all duration-300 hover:scale-110 group",
          isOpen && "hidden"
        )}
