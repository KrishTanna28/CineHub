...
export class AIPointsCalculator {
  
  /**
   * Main entry point: Calculate points with AI enhancement
   */
  static async calculateReviewPoints(review, user, context = {}) {
    const breakdown = {};
    
    // 1. Base Points (deterministic logic)
    const basePoints = this.calculateBasePoints(review);
    breakdown.base = basePoints;
    
    // 2. AI Quality Score (0-1 multiplier)
    const qualityScore = await this.analyzeReviewQuality(review);
    ...
    const feedback = await this.generateFeedback(breakdown, review);
   ...
   */
  static calculateBasePoints(review) {
    let points = 0;
    const details = {};
...
Return format:
{
   ...
   * 3. Authenticity Check (sentiment vs rating alignment)
   */
  static async checkAuthenticity(review) {
    try {
      const model = genAI.getGenerativeModel({ model: 'gemini-pro' });
...
Return format:
{
   ...
   */
  static async analyzeEngagementQuality(review, context) {
    ...
    const normalizedEngagement = this.sigmoid(rawEngagement);
...
[
   ...
   * 5. Content Analysis (spoilers, duplicates, genre detection)
   */
  static async analyzeContent(review, user) {
    try {
      const model = genAI.getGenerativeModel({ model: 'gemini-pro' });
...
Return format:
{
   ...
   * 6. Behavior Modeling & Credibility Score
   */
  static calculateCredibility(user) {
    let credibility = 1.0;
    const factors = {};
   ...
   */
  static calculateFinalScore(base, quality, authenticity, engagement, content, credibility, user) {
    ...
    // Add streak bonus
    const streakBonus = this.calculateStreakPoints(user.reviewStreak || 0);
   ...
   * 8. Generate Explanatory Feedback
   */
  static async generateFeedback(breakdown, review) {
    try {
      const model = genAI.getGenerativeModel({ model: 'gemini-pro' });
...
Be encouraging and specific.`;
   ...
   */
  static calculateMultiplier(user) {
   ...
   */
  static calculateStreakPoints(streak) {
   ...
   */
  static async reevaluateReview(review, user, context) {
    // Apply time decay
    ...
    const updatedPoints = await this.calculateReviewPoints(review, user, context);
